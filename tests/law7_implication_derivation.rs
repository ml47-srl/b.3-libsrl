...
fn test_implications_derivation() {
	let mut db = match Database::by_string("(= 'true' y).") {
		SRLResult::Ok(x) => x,
		SRLResult::Err(_) => panic!("database failure!")
	};
	...
	match db.case_creation(cell_id, cell) {
		SRLResult::Ok(x) => { assert_eq!(x.to_rule_string(), "[=> (= 'false' (= 'true' x)) (= 'true' y)]."); }
		SRLResult::Err(srl_error) => panic!("panic! (5) err: {:?}", srl_error)
	}

	let cell_id1 = CellID::create(2, vec![]);
	let cell_id2 = CellID::create(3, vec![]);
	match db.implications_derivation(cell_id1, cell_id2) {
		SRLResult::Ok(x) => { assert_eq!(x.to_rule_string(), "= 'true' y."); }
		SRLResult::Err(_) => panic!("panic! (6)")
	}
}
